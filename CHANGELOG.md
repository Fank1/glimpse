...
- Reorganized the menu: the top level now keeps just the essentials (Enable, the gesture row, Open Glimpse, Mode, Include Bookmarks in Gallery) plus the Quick Actions, **Settings**, Advanced and Updates sub-menus. Most toggles (nav buttons, zoom controls, invert in night mode, captions, maximum zoom, top-menu-zone) now live under **Settings**, and **Advanced** holds *Disable irrelevant image filtering*, *Suppress "format not supported" notice*, *Disable shadows*, *Fast image switching* and *Rescan this book*.
- Switching between images is now flashless by default — no more full-area flash each time you flip with the arrows or a swipe. New **Advanced → Fast image switching** (on by default) controls it: turn it *off* to restore the clean full clear if the previous image ever ghosts through the next one (most likely on detailed maps and slower e-ink panels).
