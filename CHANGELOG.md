...
- New **Advanced → Fast image switching** (off by default): switch between images with a quick, flashless refresh instead of the clean full clear. No flash and faster, but the previous image can briefly ghost through — most noticeable on detailed maps and slower e-ink panels — so it's your call whether the speed is worth it.
